...
export default {
  // upfileType:上传文件；类型，upfileSize：文件大小，upLimit：文件个数，upMultiple：值为true或false是否支持一次多个上传，isCover: 是否可以覆盖当前图
  // customFormat: 当upfileType为custom时候才有用，格式为'jpg、jpeg'
  // info: 显示在按钮后面的文字
  props: ['upfileType', 'customFormat', 'upfileSize', 'upLimit', 'upMultiple', 'CBParams', 'info', 'isCover', 'listType', 'isDrag', 'firstFileList', 'editGoodProductLimit', 'textTypeBtn'],
  data () {
    return {
      CosUploader: null,
  ...
  methods: {
    ...
    successCB (fileInfo, result, taskId) {
      ...
      if (result.code == 0) {
        ...
        this.$message.warning(obj.file_name + '上传成功！');
        that.$emit('urlCB', obj, that.CBParams);
      }

      if (!this.editGoodProductLimit) {
        if (this.isCover) {
