<template>
  <div class="multistage-item">
   <el-checkbox
    ...
    <el-checkbox-group
      :key="multistage.id + 20"
      :data-key="multistage.id + 20"
...
export default {
  ...
  methods: {
    // 根据this.hasCheckOrganizes选中相应的checkbox
    selectedItem () {
      // 将this.hasCheckOrganizes转化成一维数组
      let areasArr = []
      for (let item of this.hasCheckOrganizes) {
        areasArr.push(item.value)
      }
      // 选中的城市如果存在该省份中，则变为选中状态
      for (let item of this.multistage.citys) {
        // this.hasCheckOrganizes.value
    ...
    handlerEmit () {
      ...
      this.$emit('checkOrganize', this.formatData)
    }
  }
}
</script>
