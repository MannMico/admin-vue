<template>
  <div class="flows-box">
    ...
    <!-- 饼状图 -->
    <div class="echarts-item clearfix">
      <chart
        :options="piePV"
        :init-options="{
          width: '800px'
...
export default {
  ...
  watch: {
    ...
    //   if (newVal && newVal.length && this.module === '0') {
    //     this.compareWithPrev = false
    //     this.serachFlowsData()
    //   }
    // },
    filter: {
  ...
  methods: {
    ...
    changeSearchCompare (newVal) {
      if (newVal) {
        ...
        Channels.channelFlowsCount({ ...this.searchParams, begin_at: beginAt, end_at: beginAt })
          .then(res => {
            if (res.code === 300000) {
              // 将昨日数据拼到line中
    ...
    getPrevDayLineOptions (result) {
      ...
      for (let item of byTime) {
        lineOption.series[1].data.push(item.flow_num)
      }
    },
    // 清除‘昨日’的数据
    ...
    // 查询渠道流量数据
    serachFlowsData () {
      Channels.channelFlowsCount(this.searchParams)
        .then(res => {
          if (res.code === 300000) {
    ...
    getChartsOptions (result) {
      ...
      let PVOption = this.getPieOptions()
      let UVOption = this.getPieOptions()
      PVOption.title.text = '各渠道PV对比'
      UVOption.title.text = '各渠道UV对比'
      // UVOption.legend.left = 'right'
      for (let item of byChannel) {
        PVOption.legend.data.push(item.channel_name || '这是异常数据')
        PVOption.series[0].data.push({
          value: item.pv_num,
          name: item.channel_name || '这是异常数据'
        })
        UVOption.legend.data.push(item.channel_name || '这是异常数据')
        UVOption.series[0].data.push({
          value: item.uv_num,
          name: item.channel_name || '这是异常数据'
        })
      }
      ...
      for (let item of byTime) {
        ...
        lineOption.series[0].data.push(item.flow_num)
      }
