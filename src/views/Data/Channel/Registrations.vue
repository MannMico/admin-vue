...
export default {
  ...
  watch: {
    ...
    //   if (newVal && newVal.length && this.module === '1') {
  ...
  methods: {
    ...
    changeSearchCompare (newVal) {
      if (newVal) {
        ...
        Channels.channelRegisterCount({ ...this.searchParams, begin_at: beginAt, end_at: beginAt })
          .then(res => {
            if (res.code === 300000) {
              // 将昨日数据拼到line中
    ...
    getPrevDayLineOptions (result) {
      ...
      for (let item of byTime) {
        lineOption.series[1].data.push(item.reg_num)
      }
    },
    // 清除‘昨日’的数据
    ...
    // 查询渠道流量数据
    serachRegsData () {
      Channels.channelRegisterCount(this.searchParams)
        .then(res => {
          if (res.code === 300000) {
    ...
    getChartsOptions (result) {
      ...
      let pieption = this.getPieOptions()
      pieption.title.text = '各渠道转化注册用户数对比'
      for (let item of byChannel) {
        pieption.legend.data.push(item.channel_name || 'test数据')
        pieption.series[0].data.push({
          value: item.reg_num,
      ...
      for (let item of byTime) {
        ...
        lineOption.series[0].data.push(item.reg_num)
      }
