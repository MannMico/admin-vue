<template>
  <div>
     <el-form :model="enrollForm" ref="enrollForm" label-width="170px" style="width: 600px;">
      ...
      <el-form-item label="可报名时间段" required>
        <el-col :span="11">
      ...
      <el-form-item>
        <el-button type="primary" :loading="loading" @click="submitForm('enrollForm', 'onSubmit')">创建</el-button>
      </el-form-item>
    </el-form>
    <!-- 参会场次 -->
    <el-dialog
...
export default {
  ...
  computed: {
    // TODO 优化此处
    /* formatEnrollForm () {
      let originForm = this.enrollForm
      for (let item of originForm.enroll_session) {
        delete item.isCheck
      }
      return originForm
    } */
  ...
  methods: {
    ...
    onSubmit () {
      ...
      this.loading = true
      Enroll.createEnroll(this.enrollForm)
        .then(data => {
          this.loading = false
          if (data.code == 300000) {
            this.myMessage('success', '报名模板创建成功！', () => {
